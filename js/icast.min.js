...
if(settings.version==1){audio.src=settings.URL+"/;type=mp3";settings.src=audio.src;var dataURL=settings.cors+"?q="+settings.URL+"/7.html";var hisURL=settings.cors+"?q="+settings.URL+"/played.html";getSH(dataURL,hisURL)}
...
else{var dataURL=settings.URL+"/stats?sid="+settings.stream_id(function($) {
    "use strict";
    ...
    $.fn.icast = function(options) {
        var settings = $.extend({ URL: "", version: "2", stream_id: 1, mount_point: "", type: "/;type=mp3", streampath: "/stream?icy=http", enable_cors: !1, cors: "https://shoutcastapps.herokuapp.com", artwork: !0, logo: "img/logo.jpg", servertitle: "My Radio Title", show_listeners: !0, src: "", volume: 0.75, autoplay: !1 }, options);
        var thisObj;
        thisObj = this;
        var audio;
        ...
        thisObj.each(function() {
            ...
            if (settings.version == 1) { audio.src = settings.URL + "/;type=mp3";
                settings.src = audio.src; var dataURL = settings.cors + "?q=" + settings.URL + "/7.html"; var hisURL = settings.cors + "?q=" + settings.URL + "/played.html";
...
})(jQuery)+"&json=1&callback=?";var hisURL=settings.URL+"/played?sid="+settings.stream_id+"&type=json&callback=?"}
getSH(dataURL,hisURL)}
...
function updateTitle(name){$(".songtitle",thisObj).text(name)}
function updateTag(data){$(thisObj).attr("data-tag",data)}
function getCover(artist,title){artist=formatArtist(artist);title=formatTitle(title);artist=encodeURI(artist);title=encodeURI(title);var url="https://itunes.apple.com/search?term=="+artist+"-"+title+"&media=music&limit=1&callback=jQuery33109694635698601393_1554907560478&_=1554907560479";$.ajax({dataType:'jsonp',url:url,success:function(data){if(data.results.length==1){cover=data.results[0].artworkUrl100;cover=cover.replace('100x100','400x400')}
...
function getImageList(artist,title,i){artist=formatArtist(artist);title=formatTitle(title);artist=encodeURI(artist);title=encodeURI(title);var url="https://itunes.apple.com/search?term=="+artist+"-"+title+"&media=music&limit=1&callback=jQuery33109694635698601393_1554907560478&_=1554907560479";$.ajax({dataType:'jsonp',url:url,success:function(data){if(data.results.length==1){cover=data.results[0].artworkUrl100;cover=cover.replace('100x100','400x400')}
else{var cover=settings.logo}
